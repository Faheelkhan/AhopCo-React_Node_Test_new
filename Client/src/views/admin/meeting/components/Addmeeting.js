...
const AddMeeting = (props) => {
    ...
    const AddData = async (values, resetForm) => {
        try {
            ...
            const meetingData = {
                ...
                startTime: new Date(values.dateTime).toISOString(),
                endTime: new Date(new Date(values.dateTime).getTime() + 60*60*1000).toISOString(),
                location: values.location,
                meetingType: 'in-person',
                organizer: user._id,
                participants: values.related === 'Contact' ? values.attendes : 
                             values.related === 'Lead' ? values.attendesLead : [],
                status: 'scheduled'
            };
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                // Verify the created meeting by fetching it directly
                try {
                    const verifyResponse = await getApi('api/meeting');
                    console.log('Verification response after creation:', verifyResponse);
                } catch (verifyError) {
    ...
    const fetchAllData = async () => {
        try {
            if (values.related === 'Contact' && !props.leadContect) {
                // Only fetch contacts if they're not already loaded
            ...
            } else if (values.related === 'Lead' && !props.leadContect) {
                // Only fetch leads if they're not already loaded
            ...
            toast.error('Failed to fetch contacts or leads');
        }
    };
